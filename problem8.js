...
আপনার কাছে একটি জাভাস্ক্রিপ্ট ফাংশন রয়েছে যার নাম হলো isValidPhotoName(photoName, imageExtensions)।
 এই ফাংশনটি দুটি প্যারামিটার গ্রহণ করে: photoName, যা একটি ছবির নাম প্রদর্শন করে, এবং imageExtensions Array, যা .jpg, .png, ইত্যাদি ছবির এক্সটেনশন গুলি ধারণ করে।
...
function isValidPhotoName(imgName, imgExtensions) {
  if (typeof imgName !== "string" || !Array.isArray(imgExtensions)) {
    return "provide valid inputs";
  } else {
    for (const item of imgExtensions) {
      if (imgName.toLowerCase().endsWith(item.toLowerCase())) {
        return true;
      }
    }
    return false;
